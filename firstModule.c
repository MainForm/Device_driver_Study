...
// 문자 장치의 이름
#define CHAR_DEV_NAME               ("test_device")
...
static int __init initModule(void){
    ...
    if(major < 0){
        printk("[test_device] Error : register_chrdev()\n");
    ...
    printk("[test_device] Success to init char device module\n");
    printk("[test_device] Major : %d\n",major);
    
    // /sys/class에 test_class 폴더 생성
    test_class = class_create("test_class");
    if(IS_ERR(test_class)){
        printk("[test_device] Error : class_create()\n");
    ...
    printk("[test_device] test_class folder is created in /sys/class\n");
    
    test_device = device_create(test_class,NULL,MKDEV(major,0),NULL,CHAR_DEV_NAME);
    if(IS_ERR(test_device)){
        printk("[test_device] Error : device_create()\n");
    ...
    printk("[test_device] %s is created in /dev\n", CHAR_DEV_NAME);
...
static void __exit exitModule(void){
    ...
    printk("[test_device] exit module\n");
}
